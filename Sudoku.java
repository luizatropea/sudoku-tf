...
class Sudoku {
    ...
    public int[][] gerarMatriz(int[][] matriz1, int[][] matriz2, int[][] matriz3) {
    ...
    private void copiar(int[][] seletor, int[][] tabuleiro) {
        for (int i = 0; i < seletor.length; i++) {
            for (int j = 0; j < seletor[i].length; j++) {
                tabuleiro[i][j] = seletor[i][j];
            }
        }
    }
